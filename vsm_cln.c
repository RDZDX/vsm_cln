...
int filledDsplByLines = 165; //0 160
	...
	if (event == VM_KEY_EVENT_UP)
	{
		...
		case VM_KEY_NUM5:
			   ...
			   break;
                        ...
                        //mre_get_fn_id(id_get_hello_string3);
                        vm_input_text3(NULL, 500, 4, save_input);
			break;
...
void mre_load_sm(void) {
        ...
        //sm_hdl = vm_sm_load("vsm_srv"); //put "vsm_srv.vsm" to Memory card\MRE\
...
void save_input(VMINT state, VMWSTR text) {
    ...
    if (state == VM_INPUT_OK && lenght > 0) {
       ...
       //myresult = parse_string(ascii_data);
       //if (myresult1 != 1) { sprintf(cmyresult, "%01.2f", myresult);}
       vertical_scrolling_text(cmyresult);
